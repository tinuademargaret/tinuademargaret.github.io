...
---
The generative verifiers papaer struck me as one of those simple yet powerful ideas: train a model to write out a rationale, commit to a verdict, and read the verdict's probability off as a score. I wanted to do more than replicate the paper, so I went looking for a problem outside the results of the paper. I wanted a problem that was much harder than the maths problems used in the paper, ideally one where the ability of the verifier to reason will be especially useful. I thought Implicit social bias seemed like a good candidate, it is a subtle problem and having a verifier being able to reason might help improve it's performance learning from a handful of examples. 
